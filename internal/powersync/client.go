// Package powersync provides the PowerSync SQLite extension and sync protocol.
package powersync
...
// ClientError represents an error from the PowerSync service.
...
func (e *ClientError) Error() string {
	if e.StatusCode > 0 {
		return fmt.Sprintf("powersync: %d: %s", e.StatusCode, e.Message)
	}
	if e.Err != nil {
		return fmt.Sprintf("powersync: %v", e.Err)
	}
	return fmt.Sprintf("powersync: %s", e.Message)
}
...
func (e *ClientError) IsAuth() bool {
