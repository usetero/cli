...
func asDatabase(t *testing.T, db DB) *database {
	t.Helper()
	d, ok := db.(*database)
	if !ok {
		t.Fatal("expected db to be *database")
	}
	return d
}
...
func TestDB_Count(t *testing.T) {
	...
	t.Run("returns zero for empty table", func(t *testing.T) {
		...
		count, err := asDatabase(t, db).Count(ctx, "items")
		if err != nil {
	...
	t.Run("returns error for nonexistent table", func(t *testing.T) {
		...
		_, err = asDatabase(t, db).Count(ctx, "nonexistent")
		if err == nil {
