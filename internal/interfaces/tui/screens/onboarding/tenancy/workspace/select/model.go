...
// Model owns workspace-selection UI state.
type Model struct {
	scope logging.Scope
	theme theme.Theme

	options []tenancy.Workspace
...
func (m *Model) SetWorkspaces(workspaces []tenancy.Workspace, selected *tenancy.Workspace) {
	m.options = append([]tenancy.Workspace(nil), workspaces...)
	selectedIndex := 0
	if selected == nil {
...
// SelectedWorkspaceID returns currently highlighted workspace id.
func (m *Model) SelectedWorkspaceID() tenancy.WorkspaceID {
...
// Update handles local workspace-selection input.
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	return m, func() tea.Msg {
		...
		m.scope.Info("workspace highlighted", "workspace_id", selectedID)
		return SelectedMsg{WorkspaceID: selectedID}
	}
}
