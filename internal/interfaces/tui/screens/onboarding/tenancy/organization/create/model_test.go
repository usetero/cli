...
func TestModel_BackspaceAndEmptySubmit(t *testing.T) {
	model := New(logging.Scope{}, theme.New(false))

	model.Update(tea.KeyPressMsg{Text: "x", Code: 'x'})
	model.Update(tea.KeyPressMsg{Code: tea.KeyBackspace})
	if got := model.Name(); got != "" {
		t.Fatalf("expected empty name after backspace, got %q", got)
	}

	_, cmd := model.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	if cmd != nil {
		t.Fatal("expected nil command for empty submit")
	}
}
