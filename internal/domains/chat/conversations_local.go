...
func (s *LocalConversationService) Create(ctx context.Context, title *string) (ConversationID, error) {
	if s == nil || s.q == nil {
		return "", fmt.Errorf("chat local conversation service is not initialized")
	}
...
func (s *LocalConversationService) List(ctx context.Context) ([]Conversation, error) {
	if s == nil || s.q == nil {
		return nil, fmt.Errorf("chat local conversation service is not initialized")
	}
